...
export default function RequestedRides() {
  ...
  const loadAddresses = async (ridesData) => {
    try {
      ...
      // This stores promises immediately.
      // So duplicate coordinates won't create
      // multiple API requests.
      const requestCache = new Map();

      const getCachedAddress = (lat, lng) => {
        const key = `${Number(lat).toFixed(6)},${Number(
          lng
        ).toFixed(6)}`;
        ...
        // Create request immediately and store
        // the Promise in cache.
        const request =
  ...
  const fetchRequestedRides = async () => {
    try {
      ...
      const response =
        await getRequestedRides();

      console.log(
        "REQUESTED RIDES RESPONSE:",
        response
      );
      ...
      // IMPORTANT:
      // Do NOT await loadAddresses here.
      //
      // This allows the table to appear immediately.
      // Addresses will load in the background.
      loadAddresses(requestedRides);

      // Stop loading immediately after rides
      // are received.
      setLoading(false);

    } catch (err) {
      console.error(
        "REQUESTED RIDES ERROR:",
        err
      );
    ...
    <div className="requested-rides-container">
      ...
      <div className="page-header">
        ...
        <div>
          ...
          <p>
            View all rides waiting for driver
            acceptance.
          </p>
        </div>
